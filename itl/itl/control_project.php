...
<!-- 还款状态 -->    
   <th>
	<a href="<?php echo $pagename; ?>?<?php echo $current_url; ?>&sort=project_status&order=<?php 
	  if ( $sortlist <> "tk_hk_pd"){
	  echo "DESC";
	  }else if( $sortlist == "tk_hk_pd" && $orderlist == "DESC"){
	  echo "ASC";
	  } 
	  else {
	  echo "DESC";
	  ...
	  ?>" 
	  <?php 
	  if($sortlist=="tk_hk_pd" && $orderlist=="ASC"){
	  echo "class='sort_asc'";
	  } else if ($sortlist=="tk_hk_pd" && $orderlist=="DESC"){
	  echo "class='sort_desc'";
	  }
...
<tbody>
  <?php do { ?>
   ...
   <!-- 还款状态 -->   
      <td>
      <?php if(! $row_Recordset1['project_hk_pd'] ){
	  $huankuan_zt = "未报销";
	  } else {
	  $huankuan_zt = $row_Recordset1['project_hk_pd'];
	  }
